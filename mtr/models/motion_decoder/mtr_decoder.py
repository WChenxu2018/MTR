...
class MTRDecoder(nn.Module):
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        ...
        return ret_obj_feature, ret_pred_dense_future_trajs #torch.Size([18, 100, 512]) torch.Size([18, 100, 80, 7])
    ...
    def apply_transformer_decoder(self, center_objects_feature, center_objects_type, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos):
        intention_query, intention_points = self.get_motion_query(center_objects_type) #torch.Size([64, 18, 512]) torch.Size([64, 18, 2])
        query_content = torch.zeros_like(intention_query) #torch.Size([64, 18, 512])
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            obj_query_feature = self.apply_cross_attention( #torch.Size([64, 18, 512])
            ...
            # update
            pred_waypoints = pred_trajs[:, :, :, 0:2] #使用预测出的轨迹点当作输入
        ...
        return pred_list # 6*2*torch.Size([18, 64])
    ...
    def forward(self, batch_dict):
        ...
        obj_feature[obj_mask] = obj_feature_valid #torch.Size([18, 100, 512])
        ...
        map_feature[map_mask] = map_feature_valid #torch.Size([18, 100])

        # dense future prediction obj_pos: torch.Size([18, 100, 3])
